...
<nav class="border-b border-gray-200 bg-white" x-data="{ mobileOpen: false }">
    <div class="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
        <div class="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div class="hidden items-center gap-1 md:flex">
                ...
                <a href="{{ route('records.index') }}" class="{{ $navLink }} {{ request()->routeIs('records.index') ? $navActive : $navIdle }}">{{ __('Records') }}</a>
    ...
    <div id="budget-mobile-nav" x-show="mobileOpen" x-cloak x-transition class="border-t border-gray-100 md:hidden">
        <div class="mx-auto max-w-6xl space-y-1 px-4 py-3 sm:px-6">
            ...
            <a href="{{ route('records.index') }}" class="block rounded-lg px-3 py-2 text-base font-medium {{ request()->routeIs('records.index') ? 'bg-gray-100 text-gray-900' : 'text-gray-700 hover:bg-gray-50' }}">{{ __('Records') }}</a>
